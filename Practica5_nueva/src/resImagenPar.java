...
/**
* Programa que se encarga de realizar el suavizado de imágenes para una matriz nxn pixeles de forma paralela
...
public class resImagenPar implements Runnable {
	...
	private static void crearImagen() throws Exception {
		InputStream is = new BufferedInputStream(new FileInputStream("imagen.png"));
	...
	public static void main(String[] args) throws Exception {
		/*
		 * Random rnd = new Random(); for(int i=0;i<n;i++){ for(int j=0;j<n;j++){
	...
	private static void ejecutar() {
		...
		for (int i = 0; i < nTareas - 1; i++) {
			ex.submit(new resImagenPar(i * bloque, (i + 1) * bloque));
		}
		ex.submit(new resImagenPar((nTareas - 1) * bloque, nTareas));
		ex.shutdown();
