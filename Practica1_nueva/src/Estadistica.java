/** 
 * @author Miguel Afán Espinosa
 * @version 1.0
 * Entrada: Lee la longitud y un array de numeros introducidos por el usuario
 * Salida: Según la opción seleccionada por el usuario, imprime la media, moda, varianza o desviación típica del array introducido.
*/


import java.util.*;
public class Estadistica {

	public static void main(String[] args) {
		
		Scanner s = new Scanner (System.in);
		
		int longitud,op;
		float[] numeros;
		float suma=0;
		float media=0;
		float varianza=0;
		
		System.out.println("Introduzca la longitud de numeros a introducir: ");
		
		longitud=s.nextInt();
		
		numeros=new float[longitud+1];
		
		for (int i=1;i<=longitud;i++) {
			
			System.out.println("Introduzca el numero " +i+ ": ");
			
			numeros[i]=s.nextFloat();
			
			suma=suma + numeros[i];			
		}
		
		media=suma/longitud;
		
		System.out.println("1.- Media - 2.- Moda - 3.- Varianza y Desviación Tipica");
		
		op=s.nextInt();
		
		switch (op) {
		
		case 1: 
			
			
			System.out.println("La media es: "+media);
			break;
			
		case 2: 
			
			
			float frec=0;
			float moda=0;
			
			for (int j=0;j<=longitud;j++) {
				if (numeros[j]>frec) {
					
					frec=numeros[j];
					moda=numeros[j];
				}
						
				
			}
							
			System.out.println("La moda es: " +moda);
			
			
			break;
			
		case 3: 
			
			double cont = 0;
			

			for (int i = 1; i <= longitud; i++) {
				
				cont = cont + Math.pow((numeros[i] - media), 2);
				
			}
			varianza = (float) (cont / (longitud - 1));
			System.out.println("La varianza es: " +varianza);
		
	
			double desviacion=0;
			desviacion= Math.sqrt(varianza);
			System.out.println("La desviación es: " +desviacion);
			break;
			
		default: System.out.println("Opción Incorrecta");
		
		}
	}

}
